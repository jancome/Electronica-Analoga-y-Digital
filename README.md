# Electrónica Analógica y Digital – 2026-2

Repositorio académico de la asignatura **Electrónica Analógica y Digital**, diseñado para organizar el material de clase, guías de laboratorio, talleres, simulaciones, rúbricas, cronograma, proyecto final y documentos principales del curso.

## Propósito

Este repositorio funcionará como aula de apoyo para el segundo semestre de 2026. Su objetivo es que los estudiantes encuentren de forma ordenada los contenidos teóricos, actividades prácticas, guías de laboratorio, documentos de evaluación y lineamientos del proyecto final.

## Unidades del curso

1. **Electrónica analógica básica**  
   Diodos, unión PN, rectificación, regulación con Zener, transistor BJT y transistor FET/MOSFET.

2. **Lógica combinacional**  
   Sistemas numéricos, aritmética binaria, compuertas lógicas, tablas de verdad, álgebra de Boole y mapas de Karnaugh.

3. **Aplicaciones combinacionales y secuenciales**  
   Sumadores, restadores, comparadores, codificadores, decodificadores, multiplexores, demultiplexores, displays, flip-flops y contadores.

## Para empezar

- [Guía rápida para estudiantes](GUIA_RAPIDA_ESTUDIANTES.md)
- [Checklist de inicio del curso](CHECKLIST_INICIO_CURSO.md)
- [Mapa general del curso](MAPA_DEL_CURSO.md)

## Navegación principal

- [Syllabus resumido](SYLLABUS.md)
- [Cronograma 2026-2](CRONOGRAMA_2026_2.md)
- [Semanas de clase](semanas/README.md)
- [Evaluación](EVALUACION.md)
- [Calendario semanal de evaluaciones](evaluaciones/calendario-evaluacion-semanal.md)
- [Control de publicación de evaluaciones](evaluaciones/CONTROL_PUBLICACION_EVALUACIONES.md)
- [Proyecto final](PROYECTO_FINAL.md)
- [Normas de clase](NORMAS_DE_CLASE.md)
- [Plantillas de entrega](plantillas/README.md)

## Documentos principales

- [Syllabus](documentos-principales/syllabus/README.md)
- [Calendario académico](documentos-principales/calendario-academico/README.md)
- [Modelo de informe de laboratorio](documentos-principales/modelo-informe-laboratorio/README.md)
- [Rúbricas](documentos-principales/rubricas/README.md)
- [Bibliografía](documentos-principales/bibliografia/README.md)
- [Enlaces de apoyo](documentos-principales/enlaces-clases/README.md)

## Guías de laboratorio

Cada guía de laboratorio se trabaja en grupos de 3 estudiantes y se compone de tres notas internas: preinforme o trabajo en clase, informe completo y aporte individual.

### Electrónica analógica

- [Lab A01 – Diodos, rectificación y Zener](guias-laboratorio/analogica/lab-a01-diodos-rectificacion-zener/README.md)
- [Lab A02 – Transistor BJT](guias-laboratorio/analogica/lab-a02-transistor-bjt/README.md)
- [Lab A03 – Transistor FET/MOSFET](guias-laboratorio/analogica/lab-a03-transistor-fet-mosfet/README.md)

### Electrónica digital

- [Lab 01 – Compuertas lógicas](guias-laboratorio/digital/lab-01-compuertas-logicas/README.md)
- [Lab 02 – Álgebra booleana](guias-laboratorio/digital/lab-02-algebra-booleana/README.md)
- [Lab 03 – Teorema de De Morgan](guias-laboratorio/digital/lab-03-teorema-demorgan/README.md)
- [Lab 04 – XOR y aplicaciones](guias-laboratorio/digital/lab-04-xor-aplicaciones/README.md)
- [Lab 05 – Sumadores y aplicaciones](guias-laboratorio/digital/lab-05-sumadores-aplicaciones/README.md)
- [Lab 06 – Comparadores y paridad](guias-laboratorio/digital/lab-06-comparadores-paridad/README.md)
- [Lab 07 – Codificadores y decodificadores](guias-laboratorio/digital/lab-07-codificadores-decodificadores/README.md)
- [Lab 08 – Multiplexores y demultiplexores](guias-laboratorio/digital/lab-08-multiplexores-demultiplexores/README.md)

## Organización semanal

El curso está organizado por semanas. Cada semana combina explicación, ejercicios guiados, laboratorio, simulación, entregas y trabajo independiente, según corresponda.

Ver carpeta: [semanas](semanas/README.md)

## Nota sobre bibliografía

Los libros base y referencias protegidas por derechos de autor se manejarán como bibliografía recomendada y capítulos sugeridos. No se recomienda subir libros completos protegidos por copyright a repositorios públicos.
